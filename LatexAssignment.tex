...
\onehalfspacing
\begin{document}
\textbf{\large{Maths \hfill Assignment \hfill IIITB\#}}
\hrule
\hspace{2 pt}
\begin{flushleft}
\textbf{Problem 1.}Show that there exists no nontrivial unramified extensions of $\mathbb{Q}$. \\
\hspace{1 pt} \\
\textbf{Solution:} If $K/\mathbb{Q}$ is a non trivial number field, then $|disc K > 1|$. But then $|disc K|$ has a prime factor so that some prime ramifies in $K$. \hfill $\qedsymbol{}$\\ 
\hspace{2 pt} \\
\textbf{Problem 2. }Compute the following: \\
(a) How does one prove a cotheorem? \\
(b) Compute $\int{\cos x dx}$. \\
(c) How does one square $\begin{pmatrix}
                         a & b \\
                         c & d \\
                        \end{pmatrix} ?$ \\
\hspace{2 pt} \\
...
\end{center}
(c) This is routine.\hfill $\qedsymbol{}$\\
\hspace{2 pt} \\
\textbf{Problem 3.} Prove that $\sqrt{2}$ is irrational. \\
\hspace{2 pt} \\
\textit{Proof.} Assume that $\sqrt{2}=\frac{a}{b}$,where $a,b \in  Z$. Without loss of generality, we may assume $gcd(a,b)=1$.Then we have \\ 
\begin{center}
$\sqrt{2}=\frac{a}{b}$
\begin{equation}
\sqrt{2}^{2}={(\frac{a}{b})}^2
...
$2a^2=b^2$ \\
\end{center}
so that$b^2$ is even, implying $b$ is even. But then $gcd(a,b)\ge2$, a contradiction. \hfill $\qedsymbol{}$ \\
\hspace{2 pt} \\ 
\hspace{2 pt}\\
\hfill 1 of 1
\end{flushleft}
